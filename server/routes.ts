...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post('/api/agents/register', async (req, res) => {
    try {
      ...
      console.log('Processing agent registration for:', agentData.email);

      // Send notification email to admin
      console.log('Sending admin notification email...');
      const adminEmailResult = await sendHostingerEmail({
        ...
        html: `
          ...
          <p><strong>Country:</strong> ${agentData.country || 'Not provided'}</p>
          <p><strong>City:</strong> ${agentData.city || 'Not provided'}</p>
      ...
      console.log('Sending confirmation email to agent...');
      const agentName = agentData.firstName || 'Agent';
      const agentEmailResult = await sendHostingerEmail({
        to: agentData.email,
